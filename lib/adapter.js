...
function use(callback) {
  ...
  setInterval(function() {
    ...
    console.log('cpu data is:', cpuData);
    ...
    for (var i in probeList) {
      if (typeof probeMetrics[probeList[i]] !== 'undefined') {
    ...
    if (typeof heapData.total !== 'undefined' &&
        typeof heapData.used !== 'undefined') {
      console.assert(heapData.total >= 0);
      console.assert(heapData.used >= 0);
